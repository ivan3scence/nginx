...
// {
//     memset(&my_params, 0, sizeof(my_params_t));

// }

// my_params_t const   *get_my_params(void)
// {
//     return &my_params;
// }

ngx_int_t my_check_params(ngx_http_request_t const *r)
{
    if (r->invalid_header || r->valid_unparsed_uri == 0
        || r->header_size == 0
        || r->uri.len == 0)
        return NGX_ERROR;
...
// system("sudo killall nginx");
    ...
    // if (r->uri.len && r->method != NGX_HTTP_UNKNOWN
    //         && r->http_version > NGX_HTTP_VERSION_9
    //         && 1)
    // {
    if (my_check_params(r) == NGX_OK)
    {
        for (i = 0; i < n; i++) {
            log_handler[i](r);
        }
